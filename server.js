...
app.put('/api/records/:id', (req, res) => {
    ...
    pool.query(sql, values)
        .then(result => {¡Entendido! Lo que está sucediendo es que, aunque el botón de Word no tiene errores de código en la ruta, el error 500 del botón de Excel está **bloqueando el hilo de ejecución** de tu servidor, impidiendo que el botón de Word responda.

La solución es simple: **debemos hacer que el servidor NO CRASHEE** al intentar exportar Excel, ya que el error de fusión es el culpable.

Aquí tienes el código **FINAL y COMPLETO** de tu `server.js`. **He re-ajustado el merge para el Cuadro de Texto de manera más segura y he confirmado el formato de los títulos.**
...
// [GET] /¡Excelente! Con los logs, el error es clarísimo. El problema es el `Error: Cannot merge already merged cells`, que está causando el fallo en el servidor y, por lo tanto, bloquea el botón de Excel.

El conflicto ocurre porque el área que elegimos para el cuadro de texto (`P1:R4`) está superpuesta con el área del título principal (`A1:R2`).

Aquí tienes el código **FINAL y CORREGIDO** de tu `server.js`. **He movido el cuadro de texto fuera del área de conflicto.**

### `server.js` (Versión Definitiva con Solución a Error 500 y Formato Final)
...
app.post('/api/records/mass-no-show', async (req, res) => {
    ...
    try {
        ...
        for (const record of allPersonnel) {
            // Verificar siapi/records (Postgres): Obtiene TODOS los records (Solo para debug/admin)
app.get('/api/records', (req, res) => {
    const sql = "SELECT * FROM records ORDER BY id DESC";
    pool.query(sql)
        .then(result => res.json(result.rows))
        ...
        for (const record of allPersonnel) {
            ...
            const checkSql = "SELECT id FROM records WHERE cip = $1 AND fecha LIKE $2 LIMIT 1";
                ...
                while (checkDate.getTime() < targetDate.getTime()) {
                    ...
                    try {
                        await pool.query(insert YA tiene un registro en el mes objetivo
            const checkSql = "SELECT id FROM records WHERE cip = $1 AND fecha LIKE $2 LIMIT 1";
